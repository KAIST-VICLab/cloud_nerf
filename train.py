...
def fps(points, n_samples):
    ...
    fps_index = furthest_point_sample(
        points, n_samples)  # 1, n_samples
...
class NeRFSystem(LightningModule):
    def __init__(self, hparams):
        ...
        if hparams.N_importance == 0:
            self.nerf_coarse = CloudNeRF(kps, fps_kps, 6 * hparams.N_emb_xyz + 3, 6 *
                                         hparams.N_emb_dir + 3)
            ...
            self.nerf_fine = CloudNeRF(kps, fps_kps, 6 * hparams.N_emb_xyz + 3, 6 *
                                       hparams.N_emb_dir + 3)
